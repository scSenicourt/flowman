...
object functions {
    ...
    def nullable_struct(cols: Column*): Column = SparkShim.column(CreateNullableStruct(cols.map(_.expr)))
    ...
    def wrap(fn:(Column) => Column, name:String) : FunctionDescription = {
        (
            ...
            exprs => fn(SparkShim.column(exprs(0))).expr
        )
    }
    def wrap(fn:(Column, Column) => Column, name:String) : FunctionDescription = {
        (
            FunctionIdentifier(name),
            new ExpressionInfo(classOf[functions].getCanonicalName, name),
            exprs => fn(SparkShim.column(exprs(0)), SparkShim.column(exprs(1))).expr
        )
    }
    def wrap(fn:(Column, Column, Column) => Column, name:String) : FunctionDescription = {
        (
            FunctionIdentifier(name),
            new ExpressionInfo(classOf[functions].getCanonicalName, name),
            exprs => fn(SparkShim.column(exprs(0)), SparkShim.column(exprs(1)), SparkShim.column(exprs(2))).expr
        )
    }
    def wrap(fn:(Column, Column, Column, Column) => Column, name:String) : FunctionDescription = {
        (
            FunctionIdentifier(name),
            new ExpressionInfo(classOf[functions].getCanonicalName, name),
            exprs => fn(SparkShim.column(exprs(0)), SparkShim.column(exprs(1)), SparkShim.column(exprs(2)), SparkShim.column(exprs(3))).expr
        )
    }
